...
HeatUp = RetTime2 - RetTime1 - 2.0
CoolDown = RetTime5 - RetTime4 - 2.0

The exported VTCC workbook uses the row-66 external endpoint route for the
summary/DB timing. RetTime3 and RetTime6 are still visible row-65 internal
endpoint evidence and should not be removed from the full report contract.
```
