...
| 参数 | 设定值 | 与通用条件的差异 |
|---|---|---|
| Heat-up interval | RetTime2 - RetTime1 - 2.0 min | Verified exported workbook row-66 external endpoint route. |
| Cool-down interval | RetTime5 - RetTime4 - 2.0 min | Verified exported workbook row-66 external endpoint route. |
...
HeatUp_Time = RetTime2 - RetTime1 - 2.0 min
CoolDown_Time = RetTime5 - RetTime4 - 2.0 min

RetTime3 and RetTime6 remain internal endpoint evidence in the visible report
layout, but the exported VTCC workbook summary/DB route uses the row-66
external endpoint timing.
```
