...
async function preflightSequence(){
  ...
  try{sequenceLog("Checking Method MD, Report MD, and sequence bindings...");const result=await api("/api/sequence/preflight",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(sequencePayload()),timeoutMs:180000});state.sequence.preflight=result;const carrier=result.carrier||{ready:true,name:"Sequence carrier",errors:[]};const items=[...result.methods.map(item=>`<div class="contract-row ${item.ready?"pass":"fail"}"><strong>${escapeHtml(item.injection)}</strong><span>${escapeHtml(item.method)}</span><b>${item.ready?"Ready":"Blocked"}</b>${[...(item.errors||[]),...(item.warnings||[])].map(value=>`<small>${escapeHtml(value)}</small>`).join("")}</div>`),`<div class="contract-row ${result.report.ready?"pass":"fail"}"><strong>Shared report</strong><span>${escapeHtml(result.report.name)}</span><b>${result.report.ready?"Ready":"Blocked"}</b>${[...(result.report.errors||[]),...(result.report.warnings||[])].map(value=>`<small>${escapeHtml(value)}</small>`).join("")}</div>`,`<div class="contract-row ${carrier.ready?"pass":"fail"}"><strong>Sequence carrier</strong><span>${escapeHtml(carrier.name||"")}</span><b>${carrier.ready?"Ready":"Blocked"}</b>${(carrier.errors||[]).map(value=>`<small>${escapeHtml(value)}</small>`).join("")}</div>`];document.querySelector("#sequencePreflight").innerHTML=items.join("")+result.warnings.map(value=>`<div class="sequence-warning">${escapeHtml(value)}</div>`).join("");document.querySelector("#generateSequence").disabled=!result.ready;setSequenceStep(3,result.ready?"Step 3: contracts are ready. Generate and runtime-check the candidate Sequence CMBX.":"Step 3: resolve the blocked Method, Report, or carrier contract before generation.");sequenceLog(result.ready?"Preflight passed.":"Preflight blocked.");}catch(error){toast(error.message);sequenceLog(`Preflight failed: ${error.message}`);}
}
