...
history and presentation objects. The production Web workflow originally used
`assets/sequence_carrier_native_test1.cmbx`, a CM-exported empty two-Injection Sequence.
It now uses `assets/sequence_carrier_native_test2.cmbx`, a second user-created native
carrier with ten blank Injection rows, two Instrument Method slots, and
one Report Template. Neither carrier contains Signal, Audit, Processing Method, or
presentation-layout objects. Generation replaces only selected Injection rows and the
Method/Report CpXm bodies while preserving the CM-authored object graph. Because all ten
native Injection names are blank, pruning selects Injection objects by native ordinal
identity rather than by name. The original 8 + 2 binding distribution and the two native
Method objects are not exposed as workflow restrictions. Selected Injection objects can
be rebound arbitrarily, and additional Instrument Method slots are created by cloning the
complete CM-authored field-18/type, field-19/object, and field-20/metadata triplet with
fresh DataContract identities and matching header nodes. The ten rows can therefore use
one shared Method, arbitrary repeated Methods, or ten independently named Method assets.
...
| Native Injection capacity | 10 total rows assignable to one through ten Instrument Methods |
| Method slot expansion | Complete DataContract triplet and header-node cloning; parser/payload verified |
| Hidden carrier objects | Exact object-count validation; native empty carrier has none |
