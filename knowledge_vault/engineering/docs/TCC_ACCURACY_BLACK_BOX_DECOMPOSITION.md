...
| Human-readable script description | `C:\ProgramData\CMBX Data Explorer Workspace\KB\FOQ\TCC\Script Description\Vanquish_TCC_Temperature_Accuracy_Method_Analysis.md` |
...
```text
Run stage starts the evaluator by setting:
  StabVars.TriggerStab1 = 1

Gradient_1 fires only after:
  StabVars.TriggerStab1 = 1
  AND ColumnComp.CC.TempReady
  has been true for 30 s

Gradient_1 body:
  StabVars.TriggerStab1 = 0
  StabVars.TriggerStab2 = 1
  evaluate upper external thermometer against current +/-0.05 C band
  evaluate lower external thermometer against current +/-0.05 C band
  increment CounterUpper/CounterLower if still inside band
  otherwise reset the corresponding counter and create a new band around the current value
  after a short Delay, set UpperReady/LowerReady according to Counter >= 4

Gradient_2 fires only after:
  StabVars.TriggerStab2 = 1
  has been true for 30 s
...
This alternates Gradient_1 and Gradient_2. The external-temperature evaluation
is therefore periodically repeated until both probes have stayed inside their
local +/-0.05 C bands for enough cycles. `TriggerStab1` is only the starting
token for this two-trigger state machine; setting it once is not the complete
accuracy readiness logic.
```
...
### 3.5 Authoring Guardrails for Generated Methods

These rules were added after manual candidate review on 2026-07-14. They are
intended for future method-script authoring and prevent two common generation
errors.

#### Guardrail A: long precondition time is not a short `Delay`

In decoded CM method tables, a stage such as:

```text
Equilibration, Time = -30.000, Duration = 30.000 [min]
```

already represents a 30 minute pre-run hold. Do not also add:

```text
Delay = 30 [min]
```

unless a real source method proves that exact syntax. In the decoded TCC source
methods, unqualified `Delay` commands such as `Delay 1`, `Delay 2`, `Delay 3`,
`Delay 5`, and `Delay 60` are short command waits/transition waits, not a
replacement for a 30 minute stage duration. A generated method must keep these
two timing mechanisms separate:

| Timing mechanism | Meaning | Authoring use |
|---|---|---|
| Negative stage time plus `Duration = ... [min]` | pre-run stage duration in minutes | use for long equilibration/precondition holds |
| `RUN Delay <number>` | source command wait/transition delay | preserve source semantics; do not assume it is minutes |

#### Guardrail B: an Accuracy point is a stability-gated macro, not `Wait + RetTime` alone

For Temperature Accuracy, a RetTime is valid only because the method has already
started the external-temperature stability trigger machinery. A generated
accuracy point must preserve this full mechanism:

```text
reset StabVars trigger/counter/range/ready variables
define Gradient_1 and Gradient_2 triggers
define upper/lower external-temperature +/-0.05 C range tracking
define CounterUpper/CounterLower and UpperReady/LowerReady
define ExitRange_Upper and ExitRange_Lower reset triggers
define Abort trigger for overlong point acquisition
set target temperature
allow transition delay where source method uses it
start the alternating trigger state machine by setting TriggerStab1=1 once
  before the first measured point
wait ColumnComp.CC.TempReady AND StabVars.LowerReady AND StabVars.UpperReady
write RetTimes.RetTimeN = System.Retention
```

`StabVars.LowerReady` and `StabVars.UpperReady` are derived states, not direct
device properties. They become true only when the external thermometers remain
inside the local `+/-0.05 C` band for the required counted cycles. Therefore a
candidate script that only sets a target and writes `RetTimeN` after a simple
wait is incomplete and must not be described as a completed Accuracy test.

#### Guardrail C: dynamic Accuracy completion invalidates fixed downstream time windows

Each Accuracy point can finish at an unknown time because the method waits for
controller readiness and external-temperature stability. Any downstream test
block that starts after an Accuracy RetTime must not end at a fixed absolute run
time unless the source method proves that the CM scheduler interprets it
relative to the dynamic wait.

For example, this is unsafe in a composite generated method:

```text
RetTimes.RetTime2 = System.Retention   # 80 C accuracy anchor at dynamic time
RetTimes.RetTime3 = System.Retention   # stability start at dynamic time
StopRun at Time = 60.000               # unsafe fixed absolute end time
RetTimes.RetTime4 = System.Retention
```

The correct generation choices are:

| Choice | When to use |
|---|---|
| Separate injection/method for the stability block | Preferred when reusing stock report timing or when CM relative timing is not verified |
| Verified relative hold/trigger anchored after `RetTime3` | Use only after confirming the CM method syntax for dynamic relative waits |
| Report-only post-processing over raw data after a logged anchor | Use when the method can collect continuously and report formulas can be redesigned around `RetTime3..RetTime4` |

If none of these choices is verified, mark the stability portion as blocked
instead of emitting a fake absolute `StopRun` time.
...
The processing probe README states that the exported XML contains the SST/IRC editor layout and columns such as `Injection Condition`, `Pass Actions`, and `Fail Actions`. The current inspector now confirms `SSTGrid` presence and empty `Controls` / zero row candidates in the decoded XML. This is strong layout evidence, but not action-row evidence. Therefore the exact condition expression, insertion action, and stop action must be verified from the non-XML sequence-command serialization or inside Chromeleon UI.
