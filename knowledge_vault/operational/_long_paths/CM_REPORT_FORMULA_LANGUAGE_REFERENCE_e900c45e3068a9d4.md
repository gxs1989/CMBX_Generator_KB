...
Do not put an Excel formula (for example `=MAX(A1:A5)`) into a CM `ReportFormulaObject`. Do not put `AUDIT.*` directly into an ordinary FormulaOne cell unless CM has created a report-formula object for that cell. The controlled pair proves existing-cell FormulaOne semantics and the V0.2 writer can replace an existing numeric, text, or formula cell through its x86 STA host; it preserves all workbook structure.
...
Treat a report table as a structured object, not as a normal `ReportFormulaObject`. V0.2 report generation preserves existing tables in the carrier. Adding or editing table columns requires a separately validated table-object schema.
...
|---|---|---|
| `SheetDescription` | Defines one named report sheet plus direct report objects | Use the exact existing `SheetName`; V0.2 does not create new sheets |
...
| FormulaOne `SpreadSheetData` | Workbook formatting, labels, images, Excel-like formulas and summary cells | V0.2 can replace verified existing single cells; preserve all other content and record structural requests for review |
...
Therefore, a web author may propose an existing-cell FormulaOne formula. It can be accepted when it uses an observed carrier pattern or a CM-created control pair. If it relies only on a Help-listed but unobserved function family, label it `OPEN VERIFICATION REQUIRED`. The compiler can persist the string; it does not certify every FormulaOne function as valid or calculate the whole workbook locally.
...
7. Use V0.2 `Workbook Value`, `Workbook Text`, or `Workbook Formula` only for existing single cells. Layout, table, range, style and other FormulaOne changes require their own control pairs.
...
- The reference CMBX must be opened/imported in CM before a generated clone is claimed usable.
